...
if ($stmt = $con->prepare('SELECT id, password FROM student1 WHERE student_id = ? or email=?')) {
	$stmt->bind_param('ss', $_POST['student_id'],$_POST['email']);
	$stmt->execute();
	$stmt->store_result();
	if ($stmt->num_rows > 0) {
		echo 'Admin exists, please choose another!';
	} else {
if ($stmt = $con->prepare('INSERT INTO student1 (student_id,name,email,phone_no, password) VALUES (?, ?, ?, ?, ?)')) {
	$password= $_POST['password'];
	$stmt->bind_param("sssss" ,$_POST['student_id'], $_POST['name'], $_POST['email'], $_POST['phone_no'], $password);
	$stmt->execute();
	...
	<div class="containter-fluid overflow-hidden ">
		<div class="row mt-3 ">
			<div class="card col-4 offset-lg-4 hero-card" style="background-color: #27bb88; color: #ffffff;">
				<h2 class="text-light display-3 d-flex justify-content-center fs-1">Student Registration</h2>
				<hr>

				<form action="" method="POST" enctype="multipart/form-data" class="form-group">
					<label for="studentid">Student ID</label>:
					<input class="form-control" type="text" id="studentid" name="student_id" placeholder="Student ID" required>
					<br>
					...
					<br><br>
					<p><a href="indexStudent.html" style="color:#ffffff;">Want to go back login page?</a></p>
				</form>
